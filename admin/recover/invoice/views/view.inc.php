<script>
    ...
    function recover(){
        var checkbox = document.getElementsByName("summit_invoice_id[]");
        var data = []; 
        if (confirm("คุณต้องการกู้คืนข้อมูล ?") == true) {
            ...
            $.post( "recover/invoice/views/index.inc.php", { 
                invoice_id:JSON.stringify(data),
                action:'recover'
            }) 
                    .done(function( data ) { 
                        console.log(data);
...
<div class="col-lg-12"> 
    <table>
        ...
        <tbody>
            ...
            ?>
                <tr class="nth-child">
                    <td><input type="checkbox" name="summit_invoice_id[]" value="<?php echo $invoice[$i]['invoice_id'];?>" /></td>
                    <td><?php echo $invoice[$i]['invoice_number'];?></td> 
                    <td><?php echo $invoice[$i]['customer_name'];?></td>
                </tr>
            <?php 
            } 
            ?>
